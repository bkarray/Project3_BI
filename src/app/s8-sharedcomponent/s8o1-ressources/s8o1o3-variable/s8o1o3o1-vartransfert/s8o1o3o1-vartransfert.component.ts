...
@Component({
  selector: 'app-s8o1o3o1-vartransfert',
  templateUrl: './s8o1o3o1-vartransfert.component.html',
  styleUrls: ['./s8o1o3o1-vartransfert.component.css']
})
export class S8o1o3o1VartransfertComponent implements OnInit {
  ...
  public VarIFU_Edit: string | number = ''
  TS_StartEditing(index: number) {
    this.editingIndex = index;
    this.VarIFU_Edit = this.Tab_VarSelect[index].FVTransf_VarTransfert
  }
  
  TS_SaveEditing(index: number) {
    this.Tab_VarSelect[index].FVTransf_VarTransfert = this.VarIFU_Edit
  ...
  TS_Add_VarSelect() {
    const Tab_NewLine = {
      FVTransf_VarTransfert: 'NewValue',
    ...
    this.S8o8o1RessourcesService.TS_Sce_Post_VarTransfert(Tab_NewLine).subscribe(response => {
      ...
      console.log("ID de la nouvelle ligne:", )
  ...
  TS_Update() {
    console.log('this.Tab_Type',this.Tab_Type)
    this.Tab_VarSelect.forEach(item => {
      if(item.VarType && item.selectedVarType) {
        const correspondingType = this.Tab_Type.find(typeItem => typeItem.FVType_VarType === item.selectedVarType);
        if(correspondingType) {
          item.FVTransf_FVType_Type = correspondingType.FVType_Id
        }
      }
    })
    console.log('this.Tab_VarSelect1',this.Tab_VarSelect)
