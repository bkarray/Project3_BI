...
@Component({
  selector: 'app-s8o1o3-vartransfert',
  templateUrl: './s8o1o3-vartransfert.component.html',
  styleUrls: ['./s8o1o3-vartransfert.component.css']
})
export class S8o1o3VartransfertComponent implements OnInit {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['Input_Var_Selected']) {
      /*this.TS_GetData_From_DB()
      setTimeout(() => {
        if (this.Input_Var_Selected === 2) {
          this.Tab_VarSelect = []
        }
      }, 100)*/
    }
  ...
  filterItems(arr: any[]): any[] {
    ...
    return arr.filter(item => {
      // Assurez-vous que item.FVTransf_VarTransfert est défini avant de l'utiliser
  ...
  TS_Add_VarSelect() {
    ...
    this.S8o8o1RessourcesService.TS_Sce_Post_VarTransfert(Tab_NewLine).subscribe(response => {
      ...
      // Création d'un nouvel objet pour l'ajouter à Tab_VarSelect
      ...
      /* ***Pour relier edit a post
      const newIndex = this.Tab_VarSelect.length - 2
      console.log('newIndex',newIndex)
      this.TS_StartEditing(newIndex)
      this.tempEditValue = ''*/
    })
  }

  TS_Remove_VarSelect(index: number) {
    /*if (this.Tab_VarSelect.length > 0) {
      this.Tab_VarSelect.splice(index, 1)
      this.S8o8o1RessourcesService.TS_Sce_Delete_VarTransfert(this.Tab_VarSelect).subscribe(response => {
  ...
  TS_Update() {
    console.log('this.Tab_VarSelect2',this.Tab_VarSelect)
